...
const StationLandfill = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_BASEURL}/waste-transfers/`, {
  ...
  return (
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl flex flex-col justify-center items-center">
      ...
      {
        finalData.DriveDistance && <div className="w-full flex flex-col">
            ...
            >
              {allVehicles.map((one) => (
                <option key={one.VehicleID} value={one.VehicleID}>
                  {one.Type} | {one.Capacity} Tons | {one.FuelCostLoaded}(Cost Loaded) | {one.FuelCostUnloaded}(Cost Unloaded) 
                </option>
              ))}
            </select>
          </div>
